...
// Archivo: inversion-de-giro.ino
// Arraque directo de motor Trifásico con inversión de giro.
...
void loop() { 
  ...
  if( digitalRead(AntiHor) ) {    // Arranque Antihorario   
     ...
     while( !digitalRead(AntiHor) ) { 
      ...
      digitalWrite(Rojo,     LOW);    // Desactiva Luz Roja
      digitalWrite(VerdeAH, HIGH);    // Activa Luz Verde Antihoraria
      digitalWrite(Amarillo, LOW);    // Desactiva Luz Amarilla
 ...
 if( digitalRead(Stop) ) {     // Parada
    ...
    digitalWrite(Verde,    LOW);   // Apaga Luz Verde
    digitalWrite(VerdeAH,  LOW);   // Apaga Luz Verde Antihoraria
    digitalWrite(Amarillo, LOW);   // Apaga Luz Amarillo
  ...
  while(digitalRead(ReleTerm) ) { 
    ...
    digitalWrite(Rojo,      LOW);   // Desactiva Luz Roja
    digitalWrite(VerdeAH,   LOW);   // Desactiva Luz Verde Antihoraria
    digitalWrite(Amarillo, HIGH);   // Activa Luz Amarilla
